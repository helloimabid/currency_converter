const apiKey =  "f09aa9bdfd99d61e536079b3"
...
btn.addEventListener("click", async (evt) => {
    ...
    const baseURL = `${baseUrl}${fromcurr.value.toUpperCase()}`;

    try {
        let response = await fetch(baseURL);
        let data = await response.json();
        const rate = data.conversion_rates[tocurr.value.toUpperCase()];
        const convertedAmount = amtval * rate;
        resultDiv.innerText = `Converted Amount: ${convertedAmount.toFixed(2)} ${tocurr.value.toUpperCase()}`;
    } catch (error) {
        console.error('Error fetching the conversion rate:', error);
        resultDiv.innerText = 'Error fetching the conversion rate. Please try again later.';
    }
});
